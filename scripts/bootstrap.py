...
def validate_bootstrap_contract(root: Path) -> dict[str, object]:
    return load_contract_version(root)


def write_bootstrap_documents(root: Path, state: dict[str, object]) -> None:
    state_path = root / ".agent-state.json"
    progress_path = root / "PROGRESS.json"
