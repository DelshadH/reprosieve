        ...
        for gate_spec in docs["registry"]["gates"]:
            ...
            for reference in state["evidence"]:
                try:
                    ...
                    if proof_started_at < execution["started_at_parsed"]:
                        raise ValueError(f"{gate}: proof predates immutable execution start")
                    if proof_finished_at.timestamp() > now.timestamp() + 5 * 60:
                        raise ValueError(f"{gate}: proof timestamp is implausibly in the future")
                    assert_execution_deadline(state=execution, deadline_hours=owner["deadline_hour"], label=f"{gate} / {owner['id']}", timestamp=proof_finished_at)
