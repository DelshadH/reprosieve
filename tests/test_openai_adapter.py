...
def test_multiple_completed_traces_never_publish_a_partial_capture(tmp_path: Path) -> None:
    class FakeTrace:
        workflow_name = "fake"
        group_id = None
        metadata: ClassVar[dict[str, object]] = {}
    ...
    for trace_id in ("trace_one", "trace_two"):
        ...
        processor.on_trace_end(fake)
    processor.shutdown()

    assert not output.exists()
    assert processor.completed_capsules == ()
    assert processor.failure_reason == "capture output supports exactly one completed trace"
