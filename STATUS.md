...
- Canonical history: the reviewed migration and release-preparation stack is
  merged on protected public default `main`; the former lineage is preserved
  under recovery refs.
...
- Evidence: required push CI, CodeQL, fresh-clone verification, and the
  attestation-bound final-evidence workflow must all pass at the exact
  candidate commit. The workflow's decision receipt is the commit-bound
  release record.
- Naming: the final collision check found an active, exact RunSieve /
  ...
  public release surface and canonical GitHub repository were renamed to
  ReproSieve / `reprosieve`; the immutable contract-v2 audit lineage remains
  identified as `runsieve`.
