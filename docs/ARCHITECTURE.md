...
- `replay.py`: deterministic recorded-output materialization; it does not execute application code.
- `predicate.py`: constrained offline predicate trials over a capsule and materialized values.
...
- every materialized model output maps to the retained request position;
...
Use hierarchical ddmin with memoization by canonical candidate hash. The predicate cache key includes capsule hash, predicate hash, materialization mode, environment hash, timeout, and probabilistic predicate policy. Invalid results are not cached as absent failures when the cause is transient infrastructure.
