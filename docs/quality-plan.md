...
| RS-G10 | Clean issue capsule | Linux and macOS jobs run `python reproduce.py --trust-embedded-predicate` from a fresh directory without ReproSieve, a source checkout, a provider key, or network; refusal without the explicit trust flag is tested, and the final workflow verifies GitHub attestations before aggregating their proofs. |
