...
| RS-G04 | Offline predicate reproduction | A fresh constrained trial materializes retained recorded values, executes only the declared predicate, reproduces the target predicate result, has no provider key, denies outbound network within the documented platform boundary, and leaves provider/original-tool call canaries untouched. |
...
| RS-G09 | Probabilistic predicate trials | A seeded flaky predicate fixture validates K-of-N predicate bookkeeping, fresh trial isolation, cache key, and complete attempt report; output labels the predicate result probabilistic and makes no live model claim. |
...
| RS-G12 | Release experience | Clean checkout installs, tests, reduces the killer fixture, materializes retained values, reproduces the predicate, exports the reproduction, verifies 1-minimality, and generates a ≤20-second terminal demo. |
