...
def ensure_supported_agents_version() -> str:
    try:
        ...
        raise RuntimeError("install ReproSieve with the 'openai' extra to capture SDK runs") from error
...
class ReproSieveTraceProcessor(_TracingProcessor):
    ...
    def __init__(
        ...
        *,
        ...
        self._completed_redaction: list[dict[str, JsonValue]] = []
        ...
        with self._lock:
            try:
                ...
                if self._completed:
                    ...
                    self._completed_redaction.clear()
                ...
                self._completed_redaction.append(state.report.to_json())
            except ValueError as error:
                reason = (
                    "capture payload exceeded a configured limit"
    ...
    def shutdown(self) -> None:
        with self._lock:
            ...
            if self._traces:
                ...
                self._completed.clear()
                self._completed_redaction.clear()
            if self._failure_reason is not None or self._output_path is None:
                return
            if len(self._completed) != 1 or len(self._completed_redaction) != 1:
                self._fail("capture ended without exactly one completed trace")
                self._completed.clear()
                self._completed_redaction.clear()
                return
            try:
                write_capsule(
                    self._completed[0],
                    self._output_path,
                    redaction_report=self._completed_redaction[0],
                )
